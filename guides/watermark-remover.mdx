...
- A valid **Bearer token**. See [Authentication](/authentication).
...
| `401 unauthorized` | Token expired (they last about an hour) | Fetch a fresh token — see [Authentication](/authentication). |
...
See [Errors](/errors) for the full code reference.
