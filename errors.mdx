...
The `Message` column below shows the human-facing copy for each code. Treat it as display text only: the wording can change without notice, and many endpoints replace it with a more specific reason for the same `code`. `dev` is a stable 5-digit internal code whose first three digits mirror the HTTP status. See [Response Format](/response-format) for the full envelope and the `error.type` table.
